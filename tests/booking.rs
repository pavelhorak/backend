...
pub fn test_booking() {
	...
	req.set_body(r#"{ "name": "test", "description": "test stuff", "author": "hozdic", "begin_date": "2019-12-12T12:30", "end_date": "2019-12-13T13:25", "rooms": 3, "layout": 0 }"#);
	assert_eq!(req.dispatch().body_string().unwrap(), r#"{"result": 0, "id": 1}"#);

	let v: backend::db::Reservation = serde_json::from_str(&cli.get("/rgi/events/1").dispatch().body_string().unwrap()).unwrap();
	//assert_eq!(v, );
}
